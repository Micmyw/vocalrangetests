...
import { PitchyMpmAdapter } from "./PitchyMpmAdapter";
...
describe("PitchyMpmAdapter", () => {
  it.each([55, 440, 1396.9])("detects a clean %f Hz sine", (frequencyHz) => {
    const detector = new PitchyMpmAdapter(FRAME_SIZE);
  ...
  it("rejects an unexpected frame size", () => {
    ...
    expect(() => detector.detect(new Float32Array(2048), SAMPLE_RATE)).toThrow(
      /4096/,
    );
  });
});
